...
FILE_PATH = "/opt/airflow/dags/customer_1_beneficiary.csv"
...
with DAG(
    ...
    upload_to_snowflake_stage = SnowflakeOperator(
        ...
        sql=f"""
        PUT 'file:///{FILE_PATH}'
        @raw_beneficiary_stage;
        """,
    )

    # Delete existing data from raw_beneficiary in Snowflake
    delete_raw_beneficiary = SnowflakeOperator(
        task_id="delete_from_raw_beneficiary",
        snowflake_conn_id="snowflake_default",
        sql=f"""
            DELETE FROM raw_beneficiary
    ...
    # Insert new data into raw_beneficiary
    insert_raw_beneficiary = SnowflakeOperator(
        task_id="insert_into_raw_beneficiary",
        snowflake_conn_id="snowflake_default",
    ...
    (
        ...
        >> delete_raw_beneficiary
        >> read_sql_file
        >> insert_raw_beneficiary
        >> dq_date_range
        >> ready
    )
